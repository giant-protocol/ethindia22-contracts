...
contract DePayGateway is CarefulMath {
     ...
     */
    function sendFundAmountToWallet(address payable destinationAddress) public payable {
        require(msg.value > 0);
    ...
    function sendFundAmountToEscorw(uint256 senderId, uint256 destinationId) public payable {
     ...
     */
    function claimFundAmount(uint256 senderId, uint256 destinationId, address payable destinationAddress) public onlyAdmin {
     ...
     * @param destinationId The destination unique ID generated by DPN
     */
    function refundFundAmount(uint256 senderId, uint256 destinationId) public {
        require(senderId != 0 && senderId != destinationId);
        uint256 mappingId = getUniqueId(senderId, destinationId);
        EscrowPayment storage payment = escrowFundPayments[mappingId];
        require(
     ...
     */
    function sendTokenToEscorw(uint256 senderId, uint256 destinationId, address token, uint256 amount) public {
    ...
    function claimToken(uint256 senderId, uint256 destinationId, address payable destinationAddress, address token) public onlyAdmin {
    ...
    function refundToken(uint256 senderId, uint256 destinationId, address token) public {
        require(
